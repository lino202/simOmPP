...
def main():
    ...
    parser.add_argument('--prefixOfFolders',type=str, help='Prefix used in all folders and in the paraview script', default='/results.*/')
    args = parser.parse_args()


    pattern = rf"{args.prefixOfFolders}"

    for folderPath in args.resultsFolderPath:
        start = time.time()
        variableToChange = re.search(pattern, folderPath).group()
        ...
        for i, line in enumerate(data):
            if re.search(pattern, line):
                data[i] = re.sub(pattern, variableToChange, line)
        ...
        print(f"Executing for {folderPath} finished in {time.time()-start} seconds")
