...
df = df.loc[df['Region'] != 'A']
# df = df.loc[df['Sample'] != 'DP3']
df = df.loc[df['CL (ms)'] == 1000]
df = df.loc[df['Stim Type'] == 'T']
# df = df.loc[df['Stim'] == 'B']
# CV------------------------------------------
...
plt.savefig(os.path.join(args.outFolder, "CV_sample.png"), dpi=500)
...
plt.savefig(os.path.join(args.outFolder, "APD90_sample.png"), dpi=500)
