...
def main():
    ...
    parser.add_argument('--simTypes',type=str, required=True, nargs='+', help='generally the naming prefix you used before animation, model or solution. Generally tissue|cs')
    ...
    for simType in args.simTypes:
        if not os.path.exists(os.path.join(args.resPath, simType)):
            os.mkdir(os.path.join(args.resPath, simType))

        os.symlink(os.path.join(args.resultFolders[0], simType, '{}_model.geo'.format(simType)), 
                   os.path.join(args.resPath, simType, '{}_model.geo'.format(simType)))
        
        # shutil.copy(os.path.join(args.resultFolders[0], simType, '{}_model.geo'.format(simType)), 
        #         os.path.join(args.resPath, simType, '{}_model.geo'.format(simType)))
        ...
        for i, folder in enumerate(args.resultFolders):
            ...
            directory = Path(os.path.join(folder, simType))
            time_values[str(i)] = len(list(directory.glob("*.ens")))

            total_time_values += time_values[str(i)]

            # Check the dts are equal
            with open(os.path.join(folder, simType, "{}_animation.case".format(simType)), 'r') as f:
                lines = f.readlines()
                for line in lines:
        ...
        with open(os.path.join(args.resPath, simType, "{}_animation.case".format(simType)), 'w') as f:
            f.writelines(data)


        # Map the .ens
        # We make the zero first as the following folder's zero solution is the same as the last of the previous folder solution
        srcFile = os.path.join(args.resultFolders[0], simType, '{}_solution{}.ens'.format(simType, str(0).zfill(len(str(time_values[str(0)])))))
        dstFile = os.path.join(args.resPath, simType, '{}_solution{}.ens'.format(simType, str(0).zfill(nDigits)))
        ...
        for key in time_values.keys():
            ...
            for i in range(1, time_values[key]):
                srcFile = os.path.join(args.resultFolders[int(key)], simType, '{}_solution{}.ens'.format(simType, str(i).zfill(nDigitsLocal)))
                dstFile = os.path.join(args.resPath, simType, '{}_solution{}.ens'.format(simType, str(i+count).zfill(nDigits)))
                os.symlink(srcFile, dstFile) 
                # shutil.copy(srcFile, dstFile) 
                print(srcFile, dstFile)
            count += i
    ...
    # # copy the .elc states
    # srcFile = os.path.join(args.resultFolders[-1], 'cells_state.elc')
    # dstFile = os.path.join(args.resPath, 'cells_state.elc')
    # shutil.copy(srcFile, dstFile) 

    # !!!!!remove be so so carefull
    # for folder in args.resultFolders:
